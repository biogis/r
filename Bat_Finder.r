...
# modified on 2020.12.22
#source('./Bat_Filter_VI.r')
# https://github.com/biogis/r/blob/master/Bat_Finder.r
...
#   })
# }
# source_https("https://raw.githubusercontent.com/biogis/r/master/Bat_Finder.r")
...
result <- read.csv(text='fileName, freq, Date, FullName, SNR SNE, SNR RMS, RMS, Slope, R2, median')
result.all <- read.csv(text='fileName, freq, Date, FullName, SNR SNE, SNR RMS, RMS, Slope, R2, median')
...
for(f in fns){
  ...
  if(class(a)!='try-error'){
    if(length(a@left)>0){
      ...
      # use only if your sequence is saved as XXXXX_YYYYmmdd_HHMMSS, else uncomment the line asofdate <- x
      tr <- strsplit(basename(f),"_")
      asofdate <- strptime(paste(tr[[1]][2], tr[[1]][3], sep=' '), format="%Y%m%d %H%M%S", tz='') # paste and format date and time in a single vector
      # asofdate <- 'x'
      ...
      a.filt <- fir(a, from = 8000, to = 150000)
      ...
      if(length(pks$freq)>0 & slp<0){
        ...
        if(any((pks$snr_nse) < 0.6 | (pks$snr_rms) < 0.6)){
          ...
          #
          # pp <- ggplot(dtf.freq, aes(x = specSum, y = peakBin))+
          ...
          # # #
          # # #
          # yplot <- ggplot(pwr, aes(x=freq, y=amp))+
          #   geom_line()+
          #   geom_point(data=pks,aes(x=freq,y=amp),colour='tomato')+
          #   #  geom_text(data=pks,aes(label=freq),hjust=0.2, vjust=-0.5,colour='tomato')+
          #   theme_bw(base_size = 10)+
          #   xlab('FrÃÂ©quence [kHz]')+
          #   ylab('Ampl [dB]')+
          #   theme(axis.line = element_line(colour = 'grey80'),
          #         panel.grid.major = element_blank(),
          #         panel.grid.minor = element_blank(),
